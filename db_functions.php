...
function create_grade_table($conn, $student_courses) {
  ...
  $stmt = $conn->prepare("INSERT INTO `Grade Table` (student_id, student_name, course_code, final_grade) VALUES (?, ?, ?, ?);");

  foreach ($student_courses as $student_course) {
    $student_id = $student_course['student_id'];
    $student_name = $student_course['student_name'];
    foreach ($student_course['courses'] as $course) {
      $final_grade = ($course['test_1'] * 0.2) + ($course['test_2'] * 0.2) + ($course['test_3'] * 0.2) + ($course['final_exam'] * 0.4);
      $stmt->bind_param("isss", $student_id, $student_name, $course['course_code'], $final_grade);
      $stmt->execute();
    }
  }
}
